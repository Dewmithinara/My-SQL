...
--Give me the largest total amount recorded in the Sales table

select max(Totalamount) [Maximun Amount] from [dbo].[Sales]
--MAX(TotalAmount) → Finds the highest value in the TotalAmount column of the Sales table.
--[Maximum Amount] → Renames the output column to "Maximum Amount" for readability.


--Give me the most recent sale date recorded in the Sales table
select max(SaleDate) [Max SaleDate] from [dbo].[Sales]


--Give me the payment method that comes last alphabetically in the Sales table
...
--For each product, show the maximum quantity that was sold in a single sale
...
-- For each date, show the maximum total amount recorded on that day.
